...
for i in range(len(graph.node)):
    if graph.node[i].name == "Resize_204" or graph.node[i].name == "Resize_254":
        remove_index[graph.node[i].name] = i
    elif graph.node[i].name == "Reshape_329" or graph.node[i].name == "Reshape_344" or graph.node[i].name == "Reshape_359":
        remove_index[graph.node[i].name] = i
    else:
        continue
...
onnxmltools.utils.save_model(model_simp, './yolov5m_simplify.onnx')
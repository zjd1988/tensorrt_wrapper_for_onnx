...
for i in range(len(graph.node)):
    if graph.node[i].name == "Resize_282" or graph.node[i].name == "Resize_340":
        remove_index[graph.node[i].name] = i
    elif graph.node[i].name == "Reshape_439" or graph.node[i].name == "Reshape_454" or graph.node[i].name == "Reshape_469":
        remove_index[graph.node[i].name] = i
    else:
        continue
...
onnxmltools.utils.save_model(model_simp, './yolov5x_simplify.onnx')